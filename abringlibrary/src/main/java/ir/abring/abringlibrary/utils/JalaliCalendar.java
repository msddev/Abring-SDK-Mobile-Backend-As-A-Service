...
public class JalaliCalendar extends Calendar {
    ...
    public static String getStrJalaliDate(Date gDate, boolean withClock) {
        ...
        String jalali;
        if (withClock)
            jalali = gCal.get(Calendar.HOUR_OF_DAY) + ":" + gCal.get(Calendar.MINUTE) + "-" + jDate.date + " " + strMontDay(jDate.month + 1, 100) + " " + jDate.year;
        else
            jalali = jDate.year + "/" + jDate.month + "/" + jDate.date;

        return jalali;
    }

    private static String strMontDay(int month, int WeekDay) {
        String strMonth = "", strWeekDay = "";
        switch (month) {
            case 1:
                strMonth = "فروردين";
                break;
